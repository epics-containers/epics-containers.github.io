...
- a services repo, `t03-services`, generated from
  [`services-template-helm`](https://github.com/epics-containers/services-template-helm);
- the shared beamline services `t03-epics-pvcs` and `t03-epics-opis` running in
  your cluster;
- the `ec` CLI configured to deploy and manage services in your namespace.

The worked example uses the domain **`t03`** (beamline **`bl03t`**) and the
namespace **`t03-beamline`** created in {any}`setup-kubernetes`. Substitute your
...
- A Kubernetes cluster with a namespace for your beamline.
  ...
  `t03-beamline` namespace and context.
...
|---|---|
| `domain` | `t03` |
| `description` | *(accept default — `t03 IOC Instances and Services`)* |
| `location` | `bl03t` |
| `cluster_name` | `local` *(any lower-case name for your cluster)* |
| `cluster_namespace` | `t03-beamline` |
...
Create the repo on GitHub (<https://github.com/new>) named `t03-services`, then
commit and push:
...
git remote add origin https://github.com/<your-account>/t03-services
...
# from inside the t03-services directory:
...
export EC_TARGET=t03-beamline                              # your namespace
export EC_SERVICES_REPO=https://github.com/<your-account>/t03-services
...
- **`t03-epics-pvcs`** — persistent volumes where IOCs store autosave files,
  generated GUIs and other data.
- **`t03-epics-opis`** — an nginx server that serves those GUI files.
...
```bash
ec -v deploy t03-epics-pvcs 2026.6.1
ec -v deploy t03-epics-opis 2026.6.1
```
...
```text
 ...
 t03-epics-pvcs   service   2026.6.1   True    2026-06-26T09:10:00Z
 t03-epics-opis   service   2026.6.1   True    2026-06-26T09:11:00Z
```
